...
{
	std::cout << std::endl << "Instructions:" << std::endl << "When running the macro, if you look closely at roughly the center of the screen, you will see three red pixels spaced evenly apart. These pixels mark three points the macro checks to identify if a fishing cue has come up. In the game, adjust the camera angle and zoom until the three red pixels are over the players head, where the fishing cues come up. From there, the macro should run fine. However, the macro will mistake fishing cues for other white objects (e.g. fishing line). Ensure the macro is paused or off when outside of the game, turn it on only when in." << std::endl << "Side note: the red pixels are not an exploit of the game and simply appear over the screen itself. They will disapear once you pause or exit the macro. This macro is undetectable by the game itself, but it is against the game's rules, and if you are caught using it you will likely be banned. Use it at your own risk." << std::endl << std::endl;
}
	...
	while (true)
	{
		while (!GetAsyncKeyState(VK_OEM_6))
		{
			...
			for (int x = 0; x < 3; x++)
			{
				...
				if (abs(red - green) > 5 || abs(green - blue) > 5 || red < 150 || green < 150 || blue < 150)
				{
					continue;
				}
				timer = 0;
